...
function parseJsonAttribute(html, attributeName) {
  ...
  return JSON.parse(decodeHtml(html.slice(valueStart, valueEnd)));
}

function scrapeAnnouncementBodies(html) {
  const decoded = decodeHtml(html);
  const bodies = [];
  const pattern = /"announcement_body":(\{[\s\S]*?\}),"published"/g;
  let match = pattern.exec(decoded);
  while (match) {
    try {
      bodies.push(JSON.parse(match[1]));
    } catch {
      // Steam occasionally changes embedded event payloads; skip malformed items.
    }
...
async function scrapeAnnouncementMeta() {
  ...
  const events = parseJsonAttribute(html, "data-events");
  ...
  return scrapeAnnouncementBodies(html).map((body) => ({
    title: body.headline,
    commentCount: body.commentcount ?? 0,
    forumTopicId: body.forum_topic_id,
    votesUp: body.voteupcount,
    votesDown: body.votedowncount,
  }));
}
...
async function fetchNews() {
  ...
  const items = (news.appnews?.newsitems || []).map((item) => {
    ...
    return {
      ...
      url: `https://steamcommunity.com/games/${APP_ID}/announcements/detail/${item.gid}`,
      tags: item.tags || [],
